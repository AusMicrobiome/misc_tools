...
This must have column names ['OTU','Amplicon','Kingdom','Phylum','Class','Order','Family','Genus','Species'] and be
csv.
...
expected_cols=['Sample ID','OTU','OTU Count','Amplicon','Kingdom','Phylum','Class','Order','Family','Genus','Species']
import pandas as pd
...
if tableW.values.sum() == tableL['OTU Count'].sum(): #check the table values
    ...
    taxonomy = ['OTU','Amplicon','Kingdom','Phylum','Class','Order','Family','Genus','Species']
...
if merge.drop(['Amplicon','Kingdom','Phylum','Class','Order','Family','Genus','Species'], axis=1).values.sum() ==  tableL['OTU Count'].sum():
